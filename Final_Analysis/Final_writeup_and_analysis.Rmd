...
- to code education level as a numeric variable, the following lines were added (after transforming the writted values): `df_og$edu_lv=as.character(df_og$edu_lv)`, then `df_og$edu_lv=as.numeric(df_og$edu_lv)`.
...
```{r, prediction lines, include=TRUE, results='hide', error=FALSE, warning=FALSE, message=FALSE}
...
Upon exctracting individual *AUC* (and $\kappa$) values, following the procedure detailed by Gong et al. (2017), I proceeded to exclude observations that could either not be fitted using the hyperbolic discount function, or had log-tranformed *AUC* (and $\kappa$) values that exceeded 3 standard deviations from the mean in either direction. Such observations were removed from the dataset prior to analysis, since the ezAnova function pre-registered for this analysis does not deal with incomplete observations. 
For this reason, a pre-registered mixed linear-regression was also run on the full data- reported later in the manuscript. 
...
ggplot(data=df2, aes(x=AUC))+geom_histogram() #-> strong positive skew
ggplot(data = df2, aes(x=log(AUC)))+geom_histogram() #-> looks better

#check if k is normally distributed
ggplot(data=df2, aes(x=k))+geom_histogram() #-> strong positive skew
ggplot(data = df2, aes(x=log(k)))+geom_histogram() #-> looks better
...
```{r ANOVA AUC without problem participants, include=TRUE, message=F, error=F, warning=F, results='hide'}
anova_AUC2=ezANOVA(data = df2_noNA_AUC, wid=.(subid), dv=na.omit(logAUC), within  = .(donation, kinship), between = .(age_grp), type = 3, observed = .(age_grp)) #-> using  type=3 to emulate SPSS, indicate that age_grp is observed (rather than manipulated) to get more accurate eta_sq
...
As can be seen in figure 4 above, the mean log transformed *AUC* values of the two groups are nearly identical, and there seems to be considerable variation in individual values within each group. In sum, **the main result of interest failed to replicate in this study**. 
...
#####Better analysis to test hypothesis- mixed effects linear model with planned orthogonal contrasts
